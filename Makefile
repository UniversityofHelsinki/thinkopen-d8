...
build-dev: --pre_build-dev --setup vendor docker-up ## Build development codebase and start Docker container
...
build-testing: COMPOSER_ARGS = --optimize-autoloader
build-testing: --pre_build-testing --setup vendor ## Build testing codebase

init: build-dev --site_install ## Initialise the project
...
reset-local: --pre_reset-local --setup vendor docker-up --node_modules ## Reset local environment syncing from source specified
...
fresh: --pre_fresh clean-all --setup vendor docker-up --node_modules --site_install ## Create fresh empty installation
	$(call colorecho, "\n- DONE! Login to your site with:\n")
	$(call call_in_webroot, drush uli)
	@echo "\n"
